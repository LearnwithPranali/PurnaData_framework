...
public class Supplier_stepdefinition {
	...
	public void user_enters_below_supplier_details() {
		onSupplierPage.txtBox_vendorCode.clear();
		onSupplierPage.txtBox_vendorCode.sendKeys(testContext.getMapTestData().get("VendorCode"));
		onSupplierPage.txtBox_supplierName.sendKeys(testContext.getMapTestData().get("SupplierName"));
		onSupplierPage.txtBox_contactNoSupplier.sendKeys(testContext.getMapTestData().get("contactNo"));
		onSupplierPage.txtBox_address.sendKeys(testContext.getMapTestData().get("Address"));
		onSupplierPage.txtBox_contactPerson.sendKeys(testContext.getMapTestData().get("ContactPerson"));
		onSupplierPage.txtBox_gstNo.sendKeys(testContext.getMapTestData().get("GST"));
	...
	public void supplier_should_be_created() {
		Assert.assertEquals(driver.findElement(By.xpath("//td[text()='Ram']")).getText(), "Ram");
	}
